    ...
    <div id="filter-bar">
        <template>
            ...
            <btn type="primary" @click="applyFilter()"
                v-if="!errors.any()"><?= Yii::t('app', 'Apply'); ?></btn>
        </template>
    </div>
    ...
    <div>
        ...
        <table class="table table-bordered">
                ...
                <tbody v-for='order in orders' v-if="!order.isFiltered">
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-default btn-sm" type="button" @click="editOrder(order.id)"
                                    title="<?= Yii::t('app', 'Edit'); ?>">
                                <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button>
                            <button class="btn btn-default btn-sm" type="button" @click="deleteOrder(order.id)"
        ...
        <p>
                <a class="btn btn-primary" href="/"
                   @click.prevent="createOrderForm.show=true"><?= Yii::t('app', 'Create Order'); ?></a>
        </p>
            ...
            <section>
                <modal v-model="createOrderForm.show" title="Создание заказа" :backdrop="false">
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <textarea v-model="createOrderForm.descr" name="descr" class="wide" rows="4"
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input v-model="createOrderForm.cost" type="text" name="cost" class="wide"
            ...
            <section>
                <modal v-model="editOrderForm.show" title="Редактирование заказа" :backdrop="false">
                    ...
                    <div class="control-group">
                        <label class="control-label" for="descr">Описание заказа:</label>
                        <div class="controls">
                            <textarea v-model="editOrderForm.descr" name="descr" class="wide" rows="4"
                                      v-validate="'required|max:255'" data-vv-as="Описание заказа"></textarea>
                                <alert v-if="errors.has('descr')" type="warning">{{ errors.first('descr') }}</alert>
                        </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label" for="cost">Сумма заказа:</label>
                        <div class="controls">
                            <input v-model="editOrderForm.cost" type="text" name="cost" class="wide" value="0.0"
